...
const PAPER_PUBLISH_LEAF_CLASS = "paper-publish-leaf";

// TODO: Try and abstract away more logic from actual plugin hooks here

export default class PaperPublishPlugin extends Plugin {
  ...
  cachedSettings: PaperPublishSettings | null = null;
  ...
  async onload(): Promise<void> {
    console.log(`[Paper Publish] Starting Paper Publish ${this.manifest.version}…`);
    ...
    this.registerEvent(
      this.app.workspace.on("file-menu", (menu, file: TAbstractFile) => {
        ...
        menu.addItem((item) => {
          item
            .setTitle("Create Paper Publish Project")
            .setIcon(ICON_NAME)
            .onClick(() => {
  ...
  onunload(): void {
    ...
    this.app.workspace
      .getLeavesOfType(VIEW_TYPE_PAPER_PUBLISH_EXPLORER)
  ...
  async loadSettings(): Promise<void> {
    ...
    const _pluginSettings: PaperPublishSettings = pick(
      settings,
      TRACKED_SETTINGS_PATHS
    ) as PaperPublishSettings;
    ...
    if (!_workflows) {
      console.log("[Paper Publish] No workflows found; adding default workflow.");
    ...
    const onStatusClick = () => {
      ...
      if (draft) {
        ...
        selectedTab.set("Project" as any);
      }
    };
  ...
  private async postLayoutInit(): Promise<void> {
    ...
    this.unsubscribeSelectedDraft = selectedDraft.subscribe(async (d) => {
      ...
      if (!d) {
        ...
        const currentPath = get(selectedDraftVaultPath);
        if (!currentPath && allDrafts.length > 0) {
          selectedDraftVaultPath.set(allDrafts[0].vaultPath);
        }
        return;
      }
      ...
      if (this.cachedSettings.sessionStorage === "data") {
        ...
        if (this.cachedSettings.sessionStorage === "plugin-folder") {
          if (!this.manifest.dir) {
            console.error(`[Paper Publish] No manifest.dir for saving sessions.`);
            return;
          }
  ...
  initLeaf(): void {
    if (
      this.app.workspace.getLeavesOfType(VIEW_TYPE_PAPER_PUBLISH_EXPLORER).length
    ) {
      return;
    }
    this.app.workspace.getLeftLeaf(false).setViewState({
      type: VIEW_TYPE_PAPER_PUBLISH_EXPLORER,
    });
  }
  ...
  private styleLongformLeaves(allDrafts: Draft[] = get(drafts)) {
    this.app.workspace.getLeavesOfType("markdown").forEach((leaf) => {
      if (leaf.view instanceof FileView) {
        ...
        if (draft) {
          leaf.view.containerEl.classList.add(      PAPER_PUBLISH_LEAF_CLASS);
        } else {
          leaf.view.containerEl.classList.remove(      PAPER_PUBLISH_LEAF_CLASS);
        }
      }
