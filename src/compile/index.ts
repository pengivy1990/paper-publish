...
export async function compile(
  ...
  for (let index = 0; index < workflow.steps.length; index++) {
    ...
    if (kind === null) {
      ...
      console.error(`[Paper Publish] ${error}`);
      statusCallback({
        kind: "CompileStatusError",
        error,
      });
    ...
    console.log(
      `[Paper Publish] Running compile step ${step.description.name} with context:`,
      context
    );
  ...
  console.log(
    `[Paper Publish] Compile workflow "${workflow.name}" finished with final result:`,
    currentInput
  );
