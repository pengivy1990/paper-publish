...
export const WriteToNoteStep = makeBuiltinStep({
  ...
  async compile(
    ...
    if (context.kind !== CompileStepKind.Manuscript) {
      ...
      if (openAfter) {
        console.log("[Paper Publish] Attempting to open:", filePath);

        context.app.workspace.openLinkText(filePath, "/", true).catch((err) => {
          console.error("[Paper Publish] Could not open", filePath, err);
        });
      }
...
async function writeToFile(
  ...
  console.log("[Paper Publish] Writing to:", filePath);
  ...
  switch (filePathComponent) {
    case "..": {
      ...
      if (projectPathComponents.length === 0) {
        ...
        throw new Error("[Paper Publish] Invalid path for Save as Note.");
      }
