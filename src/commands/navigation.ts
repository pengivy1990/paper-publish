...
import { VIEW_TYPE_PAPER_PUBLISH_EXPLORER } from "src/view/explorer/ExplorerPane";
...
const showLeaf = (plugin: PaperPublishPlugin) => {
  plugin.initLeaf();
...
export const revealProjectFolder: CommandBuilder = (plugin) => ({
  ...
  checkCallback(checking) {
    ...
    try {
      ...
      console.error(
        "[Paper Publish] Error calling file-explorer.revealInFolder:",
        error
      );
    }
  },
