...
export class UserScriptObserver {
  ...
  constructor(vault: Vault, userScriptFolder: string | null) {
    ...
    this.onScriptModify = debounce(() => {
      console.log(
        `[Paper Publish] File in user script folder modified, reloading scripts…`
      );
      this.loadUserSteps();
  ...
  beginObserving(): void {
    ...
    this.unsubscribeScriptFolder = pluginSettings.subscribe(async (s) => {
      ...
      if (this.userScriptFolder) {
        ...
        console.log("[Paper Publish] Cleared user script steps.");
      }
    });
  }
  ...
  async loadUserSteps(): Promise<CompileStep[]> {
    ...
    for (const file of scripts) {
      try {
        ...
        console.error(
          `[Paper Publish] skipping user script ${file} due to error:`,
          e
        );
      }
    }

    console.log(`[Paper Publish] Loaded ${userSteps.length} user script steps.`);
  ...
  private async loadScript(path: string): Promise<CompileStep> {
    ...
    if (!loadedStep) {
      console.error(
        `[Paper Publish] Failed to load user script ${path}. No exports detected.`
      );
      throw new Error(
