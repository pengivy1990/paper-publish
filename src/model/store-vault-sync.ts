...
export class StoreVaultSync {
  ...
  private async waitForSync(): Promise<void> {
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        console.log("[Paper Publish] Waiting for active sync to complete...");
        ...
        const interval = setInterval(() => {
          if (!sync.syncing) {
            ...
            console.log("[Paper Publish] Sync complete.");
            waitingForSync.set(false);
            resolve();
          }
          console.log("[Paper Publish] Sync status:", sync.syncStatus);
        }, 1000);

        // Add a timeout just in case sync never completes
        const timeout = setTimeout(() => {
          clearInterval(interval);
          console.log("[Paper Publish] Sync wait timed out");
          waitingForSync.set(false);
          resolve();
        }, this.settlingTime);
      });
  ...
  async discoverDrafts() {
    ...
    const message = `[Paper Publish] Loaded and watching projects. Found ${draftFiles.length
    ...
    if (format === "scenes") {
      ...
      if (rawScenes.length === 0) {
        ...
        try {
          ...
          console.error(
            "[Paper Publish] error manually loading frontmatter:",
            error
          );
        }
      ...
      console.log(
        `[Paper Publish] Error loading draft at ${fileWithMetadata.file.path}: invalid longform.format. Ignoring.`
      );
      return null;
    }
  }
