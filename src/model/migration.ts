...
export function determineMigrationStatus(settings: PaperPublishSettings) {
  needsMigration.set(settings.version < LONGFORM_CURRENT_PLUGIN_DATA_VERSION);
}

export async function migrate(settings: PaperPublishSettings, app: App) {
  if (settings.version >= LONGFORM_CURRENT_PLUGIN_DATA_VERSION) {
    console.log(
      `[Paper Publish] Attempted to migrate settings with version ${settings.version} > current (${LONGFORM_CURRENT_PLUGIN_DATA_VERSION}); ignoring.`
    );
    return;
  }
  ...
  while (currentVersion < LONGFORM_CURRENT_PLUGIN_DATA_VERSION) {
    ...
    if (currentVersion === 2) {
      ...
      for (const projectPath of projectPaths) {
        console.log(`[Paper Publish] Migrating ${projectPath} to Projects 2.0…`);
        ...
        try {
          ...
          console.log(
            `[Paper Publish] Error appending deprecation notice to old index file`,
            error
          );
        }
        ...
        if (drafts.length === 1) {
          ...
          console.log(`[Paper Publish] Wrote only draft to ${vaultPath}`);
        } else {
          for (const oldDraft of drafts) {
            ...
            try {
              ...
              console.log(
                `[Paper Publish] Error creating folder during migration`,
                error
              );
            }
            ...
            console.log(`[Paper Publish] Wrote ${oldDraft.name} to ${vaultPath}`);
          }
        }
      }
    }
