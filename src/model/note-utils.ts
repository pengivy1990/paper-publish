...
export async function createNoteWithPotentialTemplate(
  ...
  if (template) {
    ...
    try {
      ...
      console.error(`[Paper Publish] Error using plugin [${pluginUsed}]:`, error);
    }
    if (contents !== "") {
...
export async function createNote(
  ...
  if (!(await app.vault.adapter.exists(pathComponents.join("/")))) {
    try {
      ...
      console.error(`[Paper Publish] Failed to create new note at "${path}"`, e);
      return null;
    }
  }
...
async function createWithTemplater(
  ...
  if (!templaterPlugin) {
    console.error(
      "[Paper Publish] Attempted to use Templater plugin while disabled."
    );
    return;
  }
...
async function createWithTemplates(
  ...
  if (!corePlugin) {
    console.error(
      "[Paper Publish] Attempted to use core template plugin while disabled."
    );
    return;
  }
  // Get template body
